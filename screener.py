...
def compute_rsi_divergence(close, rsi, lookback=14):
    ...
    for i in range(1, len(price_window) - 1):
        p = price_window.iloc[i]; r = rsi_window.iloc[i]
...
def compute_pivot_points(df):
    """Standard pivot points from previous day OHLC."""
    if len(df) < 2:
        return {}
    prev = df.iloc[-2]
...
def compute_entry_signal(components, fo_signal, composite_score):
    ...
    into a single ENTRY_SIGNAL. STRONG ENTRY = 3+ confirmations.
    """
    confirmations = []
